...
- (void)createContent {
    if (self.alertStyle.actionSource == DNPopStyleActionSource_Renren) {
        [self layoutCustomSubviews_Renren];
    } else if (self.alertStyle.actionSource == DNPopStyleActionSource_RenrenPlay) {
        [self layoutCustomSubviews_RenrenPlay];
    }
}

- (void)layoutCustomSubviews_Renren {
...
- (void)layoutCustomSubviews_RenrenPlay {
    
    CGFloat previousItemMaxY = 0;
    if (self.title && self.message) {
        previousItemMaxY += self.alertStyle.headerEdge.top;
    } else if (self.title || self.message) {
        previousItemMaxY += self.alertStyle.headerEdge.top * 0.6;
    }
    ...
    if (self.title && self.message) {
        ...
        previousItemMaxY += self.alertStyle.headerEdge.top * 0.4;
    }
    ...
    if (self.alertStyle.actionSort == DNPopStyleActionSortByVertical) {
        ...
        for (NSInteger i = 0; i < self.alertActions.count; i++) {
            ...
            if (self.alertStyle.dividingLine && i < self.alertActions.count - 1) {
                ...
                line.backgroundColor = self.alertStyle.dividingLineColor.CGColor;
                CGFloat lineWidth = self.alertStyle.dividingLineHeight;
                CGFloat lineHeight = item.frame.size.height;
                CGFloat lineX = actionX;
                CGFloat lineY = previousItemMaxY;
                actionX += lineWidth;
                line.frame = (CGRect){lineX,lineY,lineWidth,lineHeight};
                [self.layer addSublayer:line];
            }
        ...
        if (self.alertStyle.dividingLine) {
            CALayer *line = [CALayer new];
            line.backgroundColor = self.alertStyle.dividingLineColor.CGColor;
            CGFloat lineWidth = maxWidth;
            CGFloat lineHeight = self.alertStyle.dividingLineHeight;
            CGFloat lineX = 0;
            CGFloat lineY = previousItemMaxY - self.alertStyle.dividingLineHeight;
            line.frame = (CGRect){lineX,lineY,lineWidth,lineHeight};
            [self.layer addSublayer:line];
        }
